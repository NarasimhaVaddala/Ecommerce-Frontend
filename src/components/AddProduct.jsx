...
export default function AddProduct() {
  ...
  {
    axios.post(`https://ecommerce-backend-ecru-mu.vercel.app/insert` , obj)
  ...
  useEffect(() => {
          ...
          setSelectedCategory("");
          setAvailableOptions([]);
          if (selectedGender)setAvailableCategories(Object.keys(data[selectedGender])); // Update available categories
          else setAvailableCategories([]); // Clear categories if no gender selected
  ...
  useEffect(() =>{
        ...
        setAvailableOptions([]);
        if (selectedCategory && selectedGender)setAvailableOptions(data[selectedGender][selectedCategory]); // Update available options 
  ...
  const onSubmit = (data) => postData({...data,category:selectedCategory,gender:selectedGender,type:selectedOption,size:size});
    ...
    <section>
      <div className="container">
        ...
        <div className="flex flex-col">
          ...
          <select className="p-2 my-2 border-2" value={selectedGender}  onChange={handleGenderChange}>
        ...
        {selectedGender && (
          <div className="flex flex-col">
            ...
            <select className="p-2 my-2 border-2" value={selectedCategory} onChange={handleCategoryChange} disabled={!selectedGender}>
        ...
        {selectedCategory && (
          <div className="flex flex-col">
            <label className="font-semibold" htmlFor="options">Type: </label>
            <select className="p-2 my-2 border-2" value={selectedOption} disabled={!selectedCategory} onChange={(e) => setSelectedOption(e.target.value)}>
                  {" "}
      ...
      {selectedOption && selectedCategory && selectedOption && (
        ...
        >
          ...
          [
            ...
            { field: "description",label: "Product Description",type: "text",},
            ...
            return (
              <div key={e.field} className="flex flex-col">
                <label className="font-semibold" htmlFor={e.field}>{e.label}</label>
                <input type={e.type} className="p-2 my-2 border-2" id={e.field}
